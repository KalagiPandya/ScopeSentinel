...
def process_text_and_persist(db: Session, project_id: UUID, raw_text: str, source: str) -> dict:
    """
    The actual pipeline-run-and-save logic, extracted so it can be reused
    by any input source (manual paste via /agent/run, Jira sync, email
    sync, future Slack/Confluence adapters, etc.) without duplicating it.

    1. Agent 1 reads `text` and extracts requirement statements
    ...
    Returns a summary dict of what was found and saved.
    """
    ...
    source_enum = SOURCE_MAP.get(source, RequirementSource.document)
    ...
    for req in extracted:
        ...
        if change["change_type"] == "new_addition":
            ...
            _save_alert(db, project_id, change_row, notification_by_text.get(text))
...
def run_agent_pipeline(
    ...
    """
    Manual entry point — paste text (meeting notes, an email body, a Jira
    ticket, anything) and run it through the pipeline. For automatic
    ingestion instead of manual paste, see /jira/sync and /email/sync.
    """
    return process_text_and_persist(
        db=db,
