        ...
        {changeList.length === 0 ? (
          ...
          <div className="space-y-2">
            {changeList.slice(0, 15).map(c => (
              <div key={c.id} className="flex items-start justify-between gap-3 text-sm py-2 border-b border-base-500 last:border-0">
                <div className="flex-1 min-w-0">
                  <span className="text-xs font-mono text-ink-500 uppercase mr-2">{c.change_type?.replace("_", " ")}</span>
                  <span className="text-ink-300 break-words">{c.risk_justification || "—"}</span>
                </div>
                {c.risk_level && <div className="shrink-0"><RiskBadge level={c.risk_level} score={c.risk_score} size="sm"/></div>}
              </div>
            ))}
          </div>
        )}
